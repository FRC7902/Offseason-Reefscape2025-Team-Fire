...
public class PhotonSim extends SubsystemBase {
    ...
    private final PhotonSubsystem[] m_cameras;
     ...
     *
     * @param m_swerveSubsystem The {@code SwerveSubsystem} object of the robot.
     * @param cameras The Camera objects created (accepts an unlimited amount of arguments).
     */
    public PhotonSim(SwerveSubsystem m_swerveSubsystem, PhotonSubsystem... cameras) {
        ...
        for (PhotonSubsystem camera : cameras) {
            Transform3d camToRobotTsf = camera.getM_camToRobotTsf();
            PhotonCamera m_camera = camera.getCamera();
            String cameraName = camera.getM_cameraName();
    ...
    public void simulationPeriodic() {
        ...
        if (PhotonConstants.enableCameraPosChange) {
            for (int i = 0; i != m_cameraSims.size(); i++) {
                ...
                PhotonSubsystem camera = m_cameras[i];
                ...
                camera.setM_camToRobotTsf(newCamToRobotTsf);

                m_visionSim.adjustCamera(m_cameraSim, camera.getM_camToRobotTsf());
            }
        }
    }
}
